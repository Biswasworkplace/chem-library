...
export default function FileManager({ user }) {
  ...
  const ensureRootFolder = useCallback(async () => {
    ...
    const { data: inserted, error: insertError } = await supabase
      .from('folders')
      .insert({ user_id: user.id, parent_id: null, name: 'Home' })
      .select('id,name,parent_id')
      .single()
    ...
    setRootFolder(inserted)
    setCurrentFolderId(inserted.id)
  }, [user.id])
  ...
  const loadCurrentFolder = useCallback(async () => {
    ...
    const [folderResponse, childResponse, filesResponse] = await Promise.all([
      ...
      supabase.from('folders').select('id,name,parent_id,created_at').eq('user_id', user.id).eq('parent_id', currentFolderId).order('created_at', { ascending: false }),
      supabase.from('files').select('*').eq('user_id', user.id).eq('folder_id', currentFolderId).order('created_at', { ascending: false })
    ])

    if (folderResponse.error) {
      setErrorMessage(folderResponse.error.message)
      setLoading(false)
      return
    }

    const folderData = folderResponse.data
    setCurrentFolder({ id: folderData.id, name: folderData.name, parent_id: folderData.parent_id })
    setChildFolders(childResponse.data || [])
    setFiles(filesResponse.data || [])

    const trail = [{ id: rootFolder?.id ?? null, name: 'Home' }]
    ...
    setLoading(false)
  }, [currentFolderId, rootFolder, user.id])
